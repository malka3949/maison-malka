import type { Messages } from "@/lib/i18n";

export const he: Messages = {
  brand: "Maison Malka",
  brandTagline: "קונדיטוריה בוטיק · ירושלים",
  brandSince: "הזמנה מראש",
  announcement: "משלוחים בירושלים · איסוף עצמי · אישור הזמנה ידני",
  navHome: "דף הבית",
  navCatalog: "קטלוג",
  navCart: "עגלה",
  navLogin: "התחברות",
  navRegister: "הרשמה",
  navLogout: "התנתקות",
  tickerItems: [
    "איסוף עצמי ללא מינימום",
    "משלוחים באזור ירושלים",
    "כל הזמנה מאושרת באופן אישי",
    "תשלום בהעברה או באיסוף",
  ],
  heroEyebrow: "Maison Malka",
  heroTitle: "קונדיטוריה בוטיק בירושלים",
  heroSubtitle: "עוגות, מאפים ומארזים — להזמנה מראש, באישור אישי",
  heroCta: "לקטלוג",
  heroSecondaryCta: "מוצרים נבחרים",
  heroImageAlt: "מאפים ועוגות של Maison Malka",
  promoCatalogLabel: "הקטלוג",
  promoCatalogTitle: "המבחר שלנו",
  promoCatalogBody: "עוגות, מאפים ומארזים מוכנים להזמנה",
  promoCatalogCta: "לצפייה בקטלוג",
  promoCatalogImageAlt: "מבחר מוצרים מהקטלוג",
  promoGiftLabel: "מארזים",
  promoGiftTitle: "לאירועים ולמתנה",
  promoGiftBody: "מארזים מורכבים בקפידה לאירועים מיוחדים",
  promoGiftCta: "למארזים",
  promoGiftImageAlt: "מארז מתנה מעוצב",
  categoriesTitle: "קטגוריות",
  categoriesSubtitle: "בחרו קטגוריה והמשיכו לגלול",
  seeFullCatalog: "לקטלוג המלא",
  featuredTitle: "נבחרים",
  featuredSubtitle: "מבחר מהקטלוג — מוכן להזמנה",
  seeAllProducts: "לכל המוצרים",
  trustDelivery: "משלוחים",
  trustDeliverySub: "באזור ירושלים",
  trustPickup: "איסוף עצמי",
  trustPickupSub: "בתיאום מראש",
  trustHandmade: "בעבודת יד",
  trustHandmadeSub: "הכנה יומית",
  trustApproval: "אישור אישי",
  trustApprovalSub: "לכל הזמנה",
  ctaFinalTitle: "להזמנה מהקטלוג",
  ctaFinalBody: "בחרו את המוצרים, שלחו בקשה — ואנחנו נחזור אליכם לאישור ולתיאום.",
  ctaFinalButton: "מעבר לקטלוג",
  catalogTitle: "קטלוג",
  catalogSubtitle: "המבחר המלא להזמנה",
  scrollPrev: "הקודם",
  scrollNext: "הבא",
  filterAll: "הכל",
  emptyCatalog: "אין מוצרים זמינים כרגע.",
  priceFrom: "החל מ־",
  addToCart: "הוספה לסל",
  viewProduct: "לפרטים",
  cartTitle: "עגלת קניות",
  cartEmpty: "העגלה ריקה.",
  continueShopping: "המשך בקניות",
  inCart: "בעגלה",
  quantity: "כמות",
  remove: "הסרה",
  checkout: "המשך להזמנה",
  subtotal: "סכום ביניים",
  total: "סה״כ",
  checkoutTitle: "השלמת הזמנה",
  fullName: "שם מלא",
  phone: "טלפון",
  email: "אימייל",
  fulfillment: "איסוף / משלוח",
  pickup: "איסוף עצמי",
  delivery: "משלוח",
  deliveryAddress: "כתובת למשלוח",
  fulfillmentDate: "תאריך מבוקש",
  paymentMethod: "אמצעי תשלום",
  bankTransfer: "העברה בנקאית",
  onPickup: "תשלום באיסוף",
  notes: "הערות",
  submitOrder: "שליחת הזמנה",
  privacyNotice:
    "הפרטים נאספים לצורך ביצוע ההזמנה ויצירת קשר. המידע מאוחסן אצל ספקי התשתית שלנו (Supabase, Vercel) ונשלחות התראות אימייל דרך Resend. אין תשלום מקוון באתר.",
  legalPrivacy: "מדיניות פרטיות",
  legalTerms: "תנאי שימוש",
  legalCancellation: "מדיניות ביטולים",
  acceptTerms: "קראתי ואני מסכים/ה לתנאי השימוש, מדיניות הפרטיות ומדיניות הביטולים",
  deliveryJerusalemNote: "משלוח זמין באזור ירושלים בלבד.",
  deliveryCostArranged: "עלות המשלוח בתיאום עם בית העסק.",
  deliveryAreaConfirm: "אני מאשר/ת שהכתובת באזור ירושלים ומבין/ה שעלות המשלוח בתיאום",
  bankTransferNextSteps:
    "בחרתם העברה בנקאית. לאחר אישור ההזמנה נשלח אליכם (באימייל) הוראות תשלום.",
  // Empty until CMS settings are filled — never show fake draft contact publicly.
  fallbackPhone: "",
  fallbackContactEmail: "ms0534113949@gmail.com",
  fallbackPickupAddress: "",
  fallbackBusinessHours: "שעות פעילות בתיאום מראש",
  orderSuccess: "ההזמנה התקבלה",
  orderSuccessBody: "בקשת ההזמנה נשמרה וממתינה לאישור.",
  orderId: "מספר הזמנה",
  backHome: "חזרה לדף הבית",
  options: "אפשרויות",
  bundleContains: "המארז כולל",
  loginTitle: "התחברות",
  registerTitle: "הרשמה",
  password: "סיסמה",
  confirmPassword: "אימות סיסמה",
  loginSubmit: "התחברות",
  registerSubmit: "יצירת חשבון",
  continueAsGuest: "המשך כאורח",
  ils: "₪",
  footerNote: "קונדיטוריה בוטיק · ירושלים",
  contactLabel: "יצירת קשר",
  hoursLabel: "שעות פעילות בתיאום",
  contactWriteUs: "שלחו הודעה",
  contactYourName: "שמכם",
  contactYourEmail: "האימייל שלכם (לחזרה)",
  contactMessage: "הודעה",
  contactSend: "שליחה",
  contactSending: "שולח…",
  contactSent: "ההודעה נשלחה. נחזור אליכם בהקדם.",
  contactClose: "סגירה",
  contactSendFailed:
    "השליחה נכשלה. בדקו שדומיין השולח מאומת ב-Resend, או ש־RESEND_DEV_TO הוא אותו מייל חנות.",
  contactNotConfigured: "שליחת מייל מהאתר לא מוגדרת כרגע. נסו שוב מאוחר יותר או התקשרו בטלפון.",
  errorGeneric: "אירעה שגיאה. נסו שוב.",
  errorRequiredOptions: "יש לבחור את כל האפשרויות הנדרשות.",
  errorEmptyCart: "העגלה ריקה.",
  errorLeadTime: "תאריך האיסוף/משלוח חייב להיות לפחות בעוד יומיים.",
  errorSaturday: "אין אספקה בשבת. בחרו יום אחר.",
  errorDeliveryAddress: "כתובת למשלוח חובה.",
  errorAcceptedTerms: "יש לאשר את תנאי השימוש והמדיניות לפני שליחה.",
  errorDeliveryArea: "יש לאשר את אזור המשלוח ועלות בתיאום.",
  errorInvalidPhone: "נא להזין מספר נייד ישראלי תקין (05X…).",
  errorRateLimited: "נשלחו יותר מדי בקשות. נסו שוב בעוד כמה דקות.",
  orderStatusLabel: "סטטוס",
  orderItems: "פריטים",
  lineTotal: "סה״כ שורה",
};
